    ...
    <section class="container mx-auto">
      <div class="flex flex-wrap my-4 md:my-12">
        <div class="w-full md:hidden px-4">
          <h2 class="text-5xl font-semibold">{{$products->name}}</h2>
          <span class="text-xl">IDR {{number_format($products->price)}}</span>
        </div>
        <div class="flex-1">
          <div class="slider">
            <div class="thumbnail">
            ...
            <div class="preview">
              <div class="item rounded-lg h-full overflow-hidden">
                <img
                  src="{{$products->galleries()->exists() ? Storage::url($products->galleries->first()->url):'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='}}"
                  alt="front"
                  class="object-cover w-full h-full rounded-lg"
                />
              </div>
        ...
        <div class="flex-1 px-4 md:p-6">
          <h2 class="text-5xl font-semibold">{{$products->name}}</h2>
          <p class="text-xl">Mulai IDR {{number_format($products->price)}}</p>
